...
dataset_path = "C:/Users/Raul/OneDrive/Escritorio/TFG/version dev/LeagueAI-development/generate_dataset"

# Randomly shuffle the list of samples in the dataset and select random test and train samples
datasets = sorted(listdir(dataset_path))
datasets_txt = sorted(listdir(dataset_path))
print(len(datasets))
print(len(datasets_txt))

for i in range(0, len(datasets)):
    # this should exist
    filename_txt = dataset_path+datasets[i].split(".")[0]+".txt"
    found = False
    for j in range(0, len(datasets_txt)):
        cur = dataset_path+datasets_txt[j].split(".")[0]+".txt"
        if cur == filename_txt:
            found = True
            break
    if found is False:
