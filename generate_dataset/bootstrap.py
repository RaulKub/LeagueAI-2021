...
masked_images_dir = "C:\\Users\\Raul\\OneDrive\\Escritorio\\TFG\\version dev\\LeagueAI-development\\generate_dataset\\DATASET_SERIO\\"
# Directory in which the masked minion images are located
masked_minions = "C:\\Users\\Raul\\OneDrive\\Escritorio\\TFG\\version dev\\LeagueAI-development\\generate_dataset\\DATASET_SERIO\\masked_minions"
# Directory in which the map backgrounds are located
map_imags_dir = "C:\\Users\\Raul\\OneDrive\\Escritorio\\TFG\\version dev\\LeagueAI-development\\generate_dataset\\mapa"
# Directory in which the map backgrounds with fog of war are located
map_fog_dir = "C:\\Users\\Raul\\OneDrive\\Escritorio\\TFG\\version dev\\LeagueAI-development\\generate_dataset\\\mapa_fog"
# Directory in which the tower images are located
tower_dir = "C:\\Users\\Raul\\OneDrive\\Escritorio\\TFG\\version dev\\LeagueAI-development\\generate_dataset\\masked_towers"
...
# -- train.txt
# -- test.txt
output_dir = "C:\\Users\\Raul\\OneDrive\\Escritorio\\TFG\\version dev\\LeagueAI-development\\generate_dataset\\DATASET_SERIO\\outputFinal"
...
scale_champions = 0.7                   #1.0 # 0.7 good
random_scale_champions = 0.1        # 0.12 is good
scale_minions = 0.6                 #1.1 good
random_scale_minions = 0.25             # 0.25 good
scale_towers = 1.0               #1.8 1.6 good
...
overlay_chance = 5
overlay_path = "C:\\Users\\Raul\\OneDrive\\Escritorio\\TFG\\version dev\\LeagueAI-development\\generate_dataset\\\mapa_ui"
...
cursor_random = 0.2
cursor_path = "C:\\Users\\Raul\\OneDrive\\Escritorio\\TFG\\version dev\\LeagueAI-development\\generate_dataset\\cursor"
...
def add_object(path, cur_image_path, object_class, bias_point, last):
    ...
    if object_class == 0: # tower
        ...
        size = int(obj_w*scale_factor), int(obj_h*scale_factor)
    elif object_class == 5: # order-tower
        scale_factor = random.uniform(scale_towers-random_scale_towers, scale_towers+random_scale_towers)
        size = int(obj_w*scale_factor), int(obj_h*scale_factor)
    elif object_class == 6: # order-minion
        scale_factor = random.uniform(scale_minions-random_scale_minions, scale_minions+random_scale_minions)
        size = int(obj_w*scale_factor), int(obj_h*scale_factor)
    elif object_class == 7: # lux
        scale_factor = random.uniform(scale_champions-random_scale_champions, scale_champions+random_scale_champions)
        size = int(obj_w*scale_factor), int(obj_h*scale_factor)
    ...
    if object_class != 6 and (object_class > 3 or object_class < 1): #object_class >= 4 or (object_class < 1 and object_class >= 0) or (object_class == -2): #Champion or structure or cursor
    ...
    if object_class >= 0:
        with open(output_dir+"\\labels\\"+filename+".txt", "a") as f:
            ...
            if (float(obj_pos_center[0]/w)) > 1 or (float(obj_pos_center[1]/h)) > 1 or float(obj_w/w) > 1 or float(obj_h/h) > 1:
                toDelete = 1
            elif (float(obj_pos_center[0]/w)) < 0 or (float(obj_pos_center[1]/h)) < 0 or float(obj_w/w) <0 or float(obj_h/h) < 0:
...
while dataset < dataset_size:
    ...
    for i in range(0, random.randint(characters_min, characters_max)):
       ...
       # if temp_obj_folder == "vayne":
       #characters.append([masked_images_dir+"\\"+temp_obj_folder+"\\"+random.choice(sorted(listdir(temp_obj_path))),4])
        if i<2:
            temp_obj_folder = "vayne"
            temp_obj_path = masked_images_dir +"\\"+temp_obj_folder
            characters.append([masked_images_dir+temp_obj_folder+"\\"+random.choice(sorted(listdir(temp_obj_path))),4]) 
        else:
            temp_obj_folder = "lux"
            temp_obj_path = masked_images_dir +"\\"+temp_obj_folder
            characters.append([masked_images_dir+temp_obj_folder+"\\"+random.choice(sorted(listdir(temp_obj_path))),7])
        #elif temp_obj_folder == "vayne":
         #   characters.append([masked_images_dir+"\\"+temp_obj_folder+"\\"+random.choice(sorted(listdir(temp_obj_path))),4]) 
    ...
    for i in range(0, random.randint(towers_min, towers_max)):
        ...
        #towers_dir = "red_tower" # Uncomment this to train with blut towers --> 
        towers_dir = random.choice(sorted(listdir(tower_dir)))
        if towers_dir == "red_tower":
            towers.append([tower_dir+"\\"+towers_dir+"\\"+random.choice(sorted(listdir(tower_dir+"\\"+towers_dir))), 0])
        else:
            towers.append([tower_dir+"\\"+towers_dir+"\\"+random.choice(sorted(listdir(tower_dir+"\\"+towers_dir))), 5])
    if verbose:
    ...
    if toDelete == 1:
            os.remove(output_dir+"\\labels\\"+filename+".txt")
            os.remove(output_dir+"\\images\\"+filename+".jpg")
            dataset = dataset-1
    if verbose:
        print("=======================================")
        dataset+=1
