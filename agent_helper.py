...
class Player():
    ...
    def click_xy(self, xy, button):
        ...
        click = True
        if click:
            win32api.SetCursorPos((int(x),int(y+top_offset)))
            if button == 0:
    ...
    def compute_hp(self, frame, draw_hp_box):
        ...
        for x_hp in range(hp_x_min, hp_x_max, 1):
            for y_hp in range(hp_y_min, hp_y_max, 1):
                ...
                if (frame[y_hp][x_hp][0] > 50 or frame[y_hp][x_hp][2] > 50) and frame[y_hp][x_hp][1] > 50:
                    all_pixels-=1
                elif frame[y_hp][x_hp][1] > 50 and frame[y_hp][x_hp][0] < 100 and frame[y_hp][x_hp][2] < 100:
                    green_pixels += 1
        if all_pixels > 0:
        ...
        if draw_hp_box:
            ...
            cv2.putText(frame, label, (int(hp_x_min +10), int(y_center + t_size[0][1]-20)), cv2.FONT_HERSHEY_PLAIN,
                        font_size, [0, 0 , 255], font_thickness)
        return hp, frame

    def compute_hp_enemy(self, frame, draw_hp_box):
        width = 138
        x_offset = 687
        height = 13
        bottom_offset = 1020
        ...
        for x_hp in range(hp_x_min, hp_x_max, 1):
            for y_hp in range(hp_y_min, hp_y_max, 1):
                # Make sure we do not count any black pixels
                if (frame[y_hp][x_hp][1] > 50 or frame[y_hp][x_hp][0] > 50) and frame[y_hp][x_hp][2] > 50:
                    all_pixels-=1
                elif frame[y_hp][x_hp][2] > 25 and frame[y_hp][x_hp][1] < 100 and frame[y_hp][x_hp][0] < 100:
                    red_pixels += 1
        hp_enemy = 100
        if all_pixels > 0:
            hp_enemy = round(red_pixels/all_pixels, 2) * 100
        self.hp_enemy = hp_enemy
        if draw_hp_box:
            cv2.rectangle(frame, (hp_x_min, hp_y_min), (hp_x_max, hp_y_max), (255, 0, 0), 2)
            label = "HP: {} %".format(self.hp_enemy)
            font_size = 1
            font_thickness = 2
            t_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_PLAIN, font_size, font_thickness)
            cv2.putText(frame, label, (int(hp_x_min +10), int(y_center + t_size[0][1]-12)), cv2.FONT_HERSHEY_PLAIN,
                        font_size, [255, 0 , 0], font_thickness)
        return hp_enemy, frame

    def detect_gold(self, frame):
        width = 138
        x_offset = 687
        height = 13
        bottom_offset = 1020
        ...
        trimmed = frame[1025:1070, 1200:1290]
        #pytesseract.image_to_string(img, config=custom_config)
        #for goldx in range(100):
           # for goldy in range(20):
              #  trimmed[goldx][goldy] = frame[int(x_center-width/2)+goldx][int(y_center)+goldy]
        trimmed = self.draw_bb(trimmed, (255, 0, 0))
        
        #print (pytesseract.image_to_string(trimmed))
        #cv2.waitKey(0)
        return trimmed
    ...
    def kite_away_from(self, closest_overall):
        ...
        if closest_overall.y < self.y:
            ...
            direction_y = -1
        self.click_xy((self.x + 75 * direction_x, self.y + 75 * direction_y), button=1)
    ...
    def attack(self, target):
        print("Attacking")
        self.click_xy((target.x, target.y), button=1)
